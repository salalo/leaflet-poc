  ...
  _prepareMapIfNeeded: function () {
    ...
    if (this._options.newPolylines) {
      ...
      that._map.on("dblclick", function (event) {
        console.log(
          "click, target=" + (event.target == that._map) + " type=" + event.type
        );
        ...
        L.Polyline.PolylineEditor([latLng], that._options, contexts).addTo(
          that._map
        );
  ...
  _addMethods: function () {
    ...
    this._init = function (options, contexts) {
      ...
      for (var i = 0; i < length; i++) {
        ...
        if (marker.context && !("originalPolylineNo" in marker.context))
          marker.context.originalPolylineNo =
            that._map._editablePolylines.length;
      }
      ...
      for (var polylineNo in that._map._editablePolylines) {
        ...
        for (var markerNo in polyline._markers) {
          ...
          if (found < that._options.maxMarkers) {
            ...
            that._setMarkerVisible(
              marker.newPointMarker,
              markerNo > 0 && bounds.contains(marker.getLatLng())
            );
          } else {
    ...
    this._setMarkerVisible = function (marker, show) {
      ...
      if (show) {
        if (!marker._visible) {
          if (!marker._map) {
            // First show for this marker:
            marker.addTo(map);
          } else {
            // Marker was already shown and hidden:
            map.addLayer(marker);
          }
          marker._map = map;
        }
    ...
    this._addMarkers = function (pointNo, latLng, fixNeighbourPositions) {
      ...
      marker.on("dragstart", function (event) {
        ...
        var previousPoint =
          pointNo && pointNo > 0
            ? that._markers[pointNo - 1].getLatLng()
            : null;
        var nextPoint =
          pointNo < that._markers.length - 1
            ? that._markers[pointNo + 1].getLatLng()
            : null;
      ...
      marker.on(that._options.addFirstLastPointEvent, function (event) {
        ...
        var pointNo = that._getPointNo(event.target);
        console.log(
          "pointNo=" + pointNo + " that._markers.length=" + that._markers.length
        );
        event.dont;
      ...
      if (that._options.customNewPointListeners)
        for (var eventName in that._options.customNewPointListeners)
          newPointMarker.on(
            eventName,
            that._options.customNewPointListeners[eventName]
          );
    ...
    this._fixAround = function (pointNoOrMarker) {
      if (typeof pointNoOrMarker == "number") var pointNo = pointNoOrMarker;
      else var pointNo = that._markers.indexOf(pointNoOrMarker);
      ...
      var nextMarker =
        pointNo < that._markers.length - 1 ? that._markers[pointNo + 1] : null;
      if (marker && previousMarker) {
        marker.newPointMarker.setLatLng([
          (previousMarker.getLatLng().lat + marker.getLatLng().lat) / 2,
          (previousMarker.getLatLng().lng + marker.getLatLng().lng) / 2,
        ]);
      }
      if (marker && nextMarker) {
        nextMarker.newPointMarker.setLatLng([
          (marker.getLatLng().lat + nextMarker.getLatLng().lat) / 2,
    ...
    this._getPointNo = function (marker) {
      for (var i = 0; i < this._markers.length; i++) {
        if (
          ...
          marker == this._markers[i].newPointMarker
        ) {
          return i;
        }
      }
